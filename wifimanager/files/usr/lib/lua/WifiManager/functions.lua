...
local function conf_sec(field,val)
  if (debug > 1) then logger(6,"{conf_sec func} BEGINNING CONFIG SECTION TEST") end
  if (debug > 2) then logger(7,"{conf_sec func} SEARCH FOR SECTION: { "..field.." }") end
  if (debug > 2) then logger(7,"{conf_sec func} SEARCH FOR VALUE: { "..val.." }") end
  local i = 0
  local sec
  local uci = uci.cursor()
  repeat
    ...
    i = i + 1
    if sec == nil then
       if (debug > 1) then logger(6,"{conf_sec func} NETWORK SECTION TEST RESULT: { FAILED }") end
       return 0 
     end
  until sec == val
  if sec then
    if ( debug > 1) then logger(6,"{conf_sec func} NETWORK SECTION TEST RESULT: { PASSED }") end
    if ( debug > 2) then logger(7,"{conf_sec func} NETWORK SECTION { "..i-1 .." }") end
    return i-1
  else
    return 0
  end
...
function sta_sec()
  if (debug > 1)  then logger(6,"{sta_sec func} BEGINNING STA SECTION TEST [ STA_SEC FUNCTION ]") end
  if (debug > 2) then logger(7,"{sta_sec func} SEARCH FOR SECTION: { STA } [ STA_SEC FUNCTION ]") end
  local i = 0
  local sec
  local uci = uci.cursor()
  repeat
    ...
    i = i + 1
   if sec == nil then 
     if (debug > 1) then logger(6,"{sta_sec func} NETWORK SECTION TEST FAILED: { NO STA FOUND } [ STA_SEC FUNCTION ]") end
     return false
   end
  until sec == "sta"
 if sec then
  if (debug > 1) then logger(6,"{sta_sec func} STA SECTION TEST PASSED: { STA FOUND } [ STA_SEC FUNCTION ]") end
  if (debug > 2) then logger(7,"{sta_sec func} STA SECTION { "..i-1 .." } [ STA_SEC FUNCTION ]") end
  return i-1
 else
  return false
 end
...
function ap_sec()
  if (debug > 1)  then logger(6,"{ap_sec func} BEGINNING AP SECTION TEST [ AP_SEC FUNCTION ]") end
  if (debug > 2) then logger(7,"{ap_sec func} SEARCH FOR SECTION: { AP } [ AP_SEC FUNCTION ]") end
  local i = 0
  local sec
  local uci = uci.cursor()
  repeat
    ...
    i = i + 1
   if sec == nil then 
     if (debug > 1) then logger(6,"{ap_sec func} AP SECTION TEST FAILED: { NO AP FOUND } [ AP_SEC FUNCTION ]") end
     return false
   end
  until sec == "ap"
 if sec then
  if (debug == 1) then logger(6,"{ap_sec func} AP SECTION TEST PASSED: { AP FOUND } [ AP_SEC FUNCTION ]") end
  if (debug == 2) then logger(7,"{ap_sec func} AP SECTION { "..i-1 .." } [ AP_SEC FUNCTION ]") end
  return i-1
 else
  return false
 end
...
local function get_ip(p)
  local hn = nixio.getaddrinfo(p,'inet','80')
  local ip = hn[1]["address"]
 return ip
end

--## TEST FOR INTERNET CONNECTION PART A ##--
function conn_test(int)
  if (log_lev > 2) then logger(7,"{conn_test func} BEGINNING NETWORK CONNECTION TEST INTERVAL: "..int) end
  for i=1, int do 
    local has_net = inet(get_ip(ping_addr),"80","1")
    if not has_net then 
      logger(1,"{conn_test func} NETWORK CONNECTION TEST [ "..i.." of "..int.." ] FAILED")
      if (i >= int) then return false end
    else
      nix.nanosleep(0,5)
      if (debug > 2) then logger(7,"{conn_test func} NETWORK CONNECTION TEST COMPLETED SUCCESSFULY ON ATTEMPT: "..i) end
      if (log_lev > 1) then logger(6,"{conn_test func} NETWORK CONNECTION TEST [ "..i.." of "..int.." ] PASSED") end
      break
    end
  end
 return true
...
local function set_client(ssid,enc,key,bssid,chn)
 ...
 if (log_lev > 1) then logger(7,"{set_client func} SETTING UP NEW CLIENT SSID: "..ssid) end
  if ssid and enc and key and bssid then
    ...
    uci:commit("wireless")
    if (log_lev > 2) then logger(6,"{set_client func} SETTING UP NEW CLIENT { PASSED } ") end
    return true
  else
    if (log_lev > 2) then logger(7,"{set_client func} SETTING UP NEW CLIENT { FAILED } ") end
    return false
  end
end
  ...
  for i,v in ipairs(ssta) do
   if not ssid or v[1] ~= ssid then
    if util.contains(csta, v[1]) then
      logger(1,"{find_network func} FOUND A MATCH "..v[1].." [ FIND_NETWORK FUNCTION ]")
      if ssid == "disabled" then 
        uci:set("wireless.@wifi-iface["..sec.."].disabled=0")
        uci:commit("wireless")
      end
      if prep_client(v[1],v[2],v[3]) then
        logger(1,"{find_network func} NETWORK: [ "..v[1].." ] HAS BEEN CONFIGURED SUCCESFULLY [ FIND_NETWORK FUNCTION ]") 
        return true 
      else
        logger(2,"{find_network func} NETWORK [ "..v[1].." ] FAILED CONECTION TEST !! [ FIND_NETWORK FUNCTION ]")
        logger(1,"{find_network func} SEARCHING FOR NEXT NETWORK !! [ FIND_NETWORK FUNCTION ]")
      end
    end
   end      
  end
  logger(2,"{find_network func} NO TRUSTED NETWORKS FOUND !! [ FIND_NETWORK FUNCTION ]")
  if ssid ~= "disabled" then
    logger(1,"{find_network func} DISABLE STA UNTIL A USABLE NETWORK IS FOUND [ FIND_NETWORK FUNCTION ]")
...
function add_ap()
  ...
  if not sec and not util.contains(wsta, ap_ssid) then
    logger(1,"{add_ap func} NO AP FOUND !! [ AP FUNCTION ]")
    logger(1,"{add_ap func} ADDING AP { "..ap_ssid.." } [ AP FUNCTION ]")
    ...
    uci:commit("wireless")
    network_reload()
    logger(1,"{add_ap func} AP [ "..ap_ssid.." ] CONFIGURED SUCCESSFULLY [ AP FUNCTION ]")
  end
end
...
function add_sta()
  ...
  if not sec then
	logger(2,"{add_sta func} ERROR NOT STA PRESENT IN THE WIRELESS CONFIG !! [ ADD_STA FUNCTION ]")
	return
  end
  ...
  if not util.contains(csta, essid) then
    ...
    uci:commit("wifimanager")
    logger(1,"{add_sta func} SSID: "..essid.." ADDED TO TRUSTED NETWORKS [ ADD_STA FUNCTION ]")
  end 
 return
end
---------------------------------------[[ END CONFIGUARATION ]]--------------------------
